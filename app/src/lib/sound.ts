...
function blip(ac: AudioContext, t0: number, b: Blip) {
  const t = t0 + (b.at ?? 0);
  const osc = ac.createOscillator();
  const g = ac.createGain();
  osc.type = b.type ?? 'sine';
  osc.frequency.setValueAtTime(b.from, t);
  if (b.to) osc.frequency.exponentialRampToValueAtTime(b.to, t + b.dur);
...
export function playSound(id: string) {
  ...
  for (const b of def.blips) blip(ac, ac.currentTime, b);
}
